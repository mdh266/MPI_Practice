...
void init(double * local_A, double * local_B, 
		int local_n, int n, int my_rank, MPI_Comm comm);
			
void print(double * local_A, int local_n, int n, int my_rank,
			MPI_Comm comm);
			
void vec_add(double * local_C, double * local_B, 
			double* local_A, int local_n);
...
int main(void)
{
	...
	local_C = malloc(n*sizeof(double));

	// Initialize the array and scatter it to the other
	// processes
	init(local_A, local_B, local_n, n, my_rank, MPI_COMM_WORLD);
	
	// PRINT THE ARRAY
	print(local_C,local_n, n, my_rank, MPI_COMM_WORLD);
	
	// Have each process add 1 to their entries
	vec_add(local_C, local_A, local_B, local_n);
		...
		int local_n, int n, int my_rank, MPI_Comm comm)
{
	...
	if(my_rank == 0)
	{
		...
		MPI_Scatter(A, local_n, MPI_DOUBLE, local_A, local_n,
					MPI_DOUBLE, 0, comm);
		MPI_Scatter(B, local_n, MPI_DOUBLE, local_B, local_n,
					MPI_DOUBLE, 0, comm);
		free(A);
		free(B);
	}
			...
			double* local_A, int local_n)
{
	...
	{
		local_C[local_i] = local_B[local_i] + local_A[local_i];
	}
}
